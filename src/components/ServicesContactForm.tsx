...
} from "@/app/actions/services-lead";

// ServicesContactForm — mirrors PilotContactForm but routes through the
// /services server action with service-line / budget / timeline fields
// instead of ERP selector. Fields chosen to give Stephan enough to reply
// with a useful next step, without turning the form into a qualification
// wall.
...
const SERVICE_LINE_OPTIONS = [
  ...
  "Custom AI",
  "Workflow automation",
  "Not sure yet — want to scope it",
];

const BUDGET_OPTIONS = [
  "Exploring — no budget agreed",
  "Small — single scoped project",
  "Medium — one team, multiple scopes",
  "Large — programme across multiple teams",
];

const TIMELINE_OPTIONS = [
  "This quarter",
  "Next quarter",
  "Flexible — whenever the right fit comes up",
];

export const ServicesContactForm: React.FC = () => {
  const [form, setForm] = useState<ServicesLeadInput>({
    name: "",
    company: "",
    email: "",
    phone: "",
  ...
  const update =
    <K extends keyof ServicesLeadInput>(key: K) =>
    (
      e:
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await submitServicesLead(form);
      if (!result.ok) {
        ...
        >
          ...
          >
            ...
            <span style={{ color: "var(--accent-2)" }}>
              you&apos;ll get a scoping call, not a sales pitch.
            </span>
          </h2>
          <p
          ...
          >
            The form lands in Stephan&apos;s inbox. Reply within one working
            day with a straight read on whether we&apos;re the right fit — or
            who might be a better fit if we&apos;re not.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="relative p-6 sm:p-10 grid lg:grid-cols-[1.4fr_1fr] gap-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <Field
                ...
                placeholder="CTO, Ops Director, Product Lead…"
              />
              <SelectField
                label="Service line"
                value={form.serviceLine ?? ""}
                onChange={update("serviceLine")}
              ...
              <div className="sm:col-span-2">
                <TextareaField
                  ...
                  placeholder="Two or three lines are plenty — e.g. 'Internal asset-tracking portal for a 40-person plant, needs to read GL from Syspro, mobile-first for floor scanning.'"
                  required
                  rows={5}
                />
              </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              >
                ...
                >
                  <Step n={1} text="Stephan reads the brief the same day." />
                  <Step
                    n={2}
                    text="Within one working day: a yes, a no, or 'let's jump on a scoping call.'"
                  />
                  <Step
                    n={3}
                    text="If it's a fit — 1–2 week scoping engagement → fixed-price quote in writing."
                  />
                </ol>
              </div>
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
              <a
                href={`mailto:${CONTACT.founder.email}?subject=${encodeURIComponent("Custom build enquiry")}`}
